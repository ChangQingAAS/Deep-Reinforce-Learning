...
sys.path.append(".")
from args.config import dueling_dqn_params as params
...
class DuelingDQN_ALGO(object):
    ...
    def init_write(self):
        for i in range(self.train_number):
            with open(self.path + "/result/DuelingDQN/result_%s.csv" % str(i), "w+", encoding="utf-8") as f:
        ...
        for train_counter in range(self.train_number):
            ...
            for n_epi in range(self.epoch):
                ...
                if n_epi % self.print_interval == 0:
                    with open(self.path + "/result/DuelingDQN/result_%s.csv" % str(train_counter),
                              "a+",
                              encoding="utf-8") as f:
