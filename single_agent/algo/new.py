...
class DDPG(object):
    def __init__(self, input_size, action_shape, hidden_size, output_size):
        ...
        self.critic = Critic(input_size + action_shape, hidden_size,
                             action_shape)
        self.critic_target = Critic(input_size + action_shape, hidden_size,
                                    action_shape)
    ...
    def update_parameters(self):
        ...
        #critic更新
        next_action_batch = self.actor_target(next_state_batch).unsqueeze(
            0).detach().view(-1, 1)
        ...
        r_target = reward_batch + GAMMA * self.critic_target(
            next_state_batch, next_action_batch).detach().view(1,
                                                               -1) * done_batch
        ...
        def soft_update(net_target, net):
            for target_param, param in zip(net_target.parameters(),
                                           net.parameters()):
